<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new chartplanrealisasidemo_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class chartplanrealisasidemo_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_labels["nomortahapan"]   = (isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["nomortahapan"])) ? $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["nomortahapan"] : "Nomor-Tahapan";
   $tab_ger_campos['keterangan'] = "on";
   $tab_def_campos['keterangan'] = "KETERANGAN";
   $tab_converte["KETERANGAN"]   = "keterangan";
   $tab_labels["keterangan"]   = (isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["keterangan"])) ? $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["keterangan"] : "KETERANGAN";
   $tab_ger_campos['nilai'] = "on";
   $tab_def_campos['nilai'] = "NILAI";
   $tab_converte["NILAI"]   = "nilai";
   $tab_labels["nilai"]   = (isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["nilai"])) ? $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['labels']["nilai"] : "NILAI";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "chartplanrealisasidemo_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['chartplanrealisasidemo']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
