...
class tracking_project_xls
{
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo   .= "_tracking_project";
      $this->Arquivo   .= ".xls";
      $this->Tit_doc    = "tracking_project.xls";
...
$_SESSION['scriptcase']['tracking_project']['contr_erro'] = 'off'; 
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
      ...
      $nmgp_order_by = $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['order_grid'];
      ...
      { 
          ...
          if ($Cada_col == "detail2" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      while (!$rs->EOF)
      {
         ...
         $this->mw_namawitel = $rs->fields[2] ;  
         $this->md_namadatel = $rs->fields[3] ;  
         $this->ms_namasto = $rs->fields[4] ;  
         $this->statusname = $rs->fields[5] ;  
         $this->tm_namamitra = $rs->fields[6] ;  
         $this->tp_projectname = $rs->fields[7] ;  
         $this->tp_releasename = $rs->fields[8] ;  
         $this->tj_namajenis = $rs->fields[9] ;  
         $this->tp_lokasi = $rs->fields[10] ;  
         $this->tp_odp = $rs->fields[11] ;  
         $this->tp_odp = (string)$this->tp_odp;
         $this->tp_planstart = $rs->fields[12] ;  
         $this->tp_planfinish = $rs->fields[13] ;  
         $this->tp_targetwaktu = $rs->fields[14] ;  
         $this->tp_targetwaktu = (string)$this->tp_targetwaktu;
         $this->tp_prosentase = $rs->fields[15] ;  
         $this->tp_prosentase =  str_replace(",", ".", $this->tp_prosentase);
         $this->tp_prosentase = (string)$this->tp_prosentase;
         $this->tp_batch = $rs->fields[16] ;  
         $this->tp_mitra = $rs->fields[17] ;  
         $this->tp_mitra = (string)$this->tp_mitra;
         $this->tp_divre = $rs->fields[18] ;  
...
$_SESSION['scriptcase']['tracking_project']['contr_erro'] = 'off'; 
   ...
   {
         $this->detail = html_entity_decode($this->detail, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->detail = strip_tags($this->detail);
         if (!NM_is_utf8($this->detail))
         {
             $this->detail = sc_convert_encoding($this->detail, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->detail, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- detail2
   function NM_export_detail2()
   {
         $this->detail2 = html_entity_decode($this->detail2, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->detail2 = strip_tags($this->detail2);
         if (!NM_is_utf8($this->detail2))
         {
             $this->detail2 = sc_convert_encoding($this->detail2, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->Xls_col) . $this->Xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValueExplicit($this->calc_cell($this->Xls_col) . $this->Xls_row, $this->detail2, PHPExcel_Cell_DataType::TYPE_STRING);
         $this->Xls_col++;
   }
   //----- tp_id
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project']['xls_file'] = $this->Xls_f;
      }
      $path_doc_md5 = md5($this->Ini->path_imag_temp . "/" . $this->Arquivo);
      $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project'][$path_doc_md5][0] = $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      $_SESSION['sc_session'][$this->Ini->sc_page]['tracking_project'][$path_doc_md5][1] = $this->Tit_doc;
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
            "http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd">
<HTML<?php echo $_SESSION['scriptcase']['reg_conf']['html_dir'] ?>>
<HEAD>
 <TITLE>List of Project Data  : Lokasi <?php echo $_SESSION['kodewitel'] ?> :: Excel</TITLE>
