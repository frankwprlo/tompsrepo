...
class resume_realisasi_project_inwitel_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['resume_realisasi_project_inwitel']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['resume_realisasi_project_inwitel']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_groupbymitradatel($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(TP_RAB), sum(TP_NILAI), sum(TP_ODP), avg(REALISASI), max(REALISASI), min(REALISASI) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['tot_geral'][7] = $rt->fields[6]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_gbmitraonly($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['contr_total_geral'] == "OK") 
      { 
   ...
   //---- 
   function quebra_geral_gbmitra($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['contr_total_geral'] == "OK") 
      { 
   ...
   //-----  tp_releasename
   function quebra_tp_releasename_groupbymitradatel($tp_releasename, $arg_sum_tp_releasename) 
   {
      ...
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_tp_releasename[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_tp_releasename[7] = (string)$rt->fields[6]; 
      $rt->Close(); 
   } 

   //-----  tm_namamitra
   function quebra_tm_namamitra_groupbymitradatel($tp_releasename, $tm_namamitra, $arg_sum_tp_releasename, $arg_sum_tm_namamitra) 
   {
      global $tot_tm_namamitra;
      $tot_tm_namamitra = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(TP_RAB), sum(TP_NILAI), sum(TP_ODP), avg(REALISASI), max(REALISASI), min(REALISASI) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq'])) 
      { 
         $nm_comando .= " where TP_RELEASENAME" . $arg_sum_tp_releasename . " and TM_NAMAMITRA" . $arg_sum_tm_namamitra ; 
      } 
      else 
      { 
         $nm_comando .= " and TP_RELEASENAME" . $arg_sum_tp_releasename . " and TM_NAMAMITRA" . $arg_sum_tm_namamitra ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tm_namamitra[0] = sc_strip_script($tm_namamitra) ; 
      $tot_tm_namamitra[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tm_namamitra[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_tm_namamitra[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_tm_namamitra[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_tm_namamitra[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_tm_namamitra[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_tm_namamitra[7] = (string)$rt->fields[6]; 
      $rt->Close(); 
   } 

   //-----  tj_namajenis
   function quebra_tj_namajenis_gbmitraonly($tj_namajenis, $arg_sum_tj_namajenis) 
   {
      global $tot_tj_namajenis;
      $tot_tj_namajenis = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(TP_RAB), sum(TP_NILAI), sum(TP_ODP), avg(REALISASI), max(REALISASI), min(REALISASI) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq'])) 
      { 
         $nm_comando .= " where TJ_NAMAJENIS" . $arg_sum_tj_namajenis ; 
      } 
      else 
      { 
         $nm_comando .= " and TJ_NAMAJENIS" . $arg_sum_tj_namajenis ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tj_namajenis[0] = sc_strip_script($tj_namajenis) ; 
      $tot_tj_namajenis[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tj_namajenis[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_tj_namajenis[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_tj_namajenis[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_tj_namajenis[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_tj_namajenis[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_tj_namajenis[7] = (string)$rt->fields[6]; 
      $rt->Close(); 
   } 

   //-----  tm_namamitra
   function quebra_tm_namamitra_gbmitra($tm_namamitra, $arg_sum_tm_namamitra) 
   {
      global $tot_tm_namamitra;
      $tot_tm_namamitra = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq'])) 
      { 
         $nm_comando .= " where TM_NAMAMITRA" . $arg_sum_tm_namamitra ; 
      } 
      else 
      { 
         $nm_comando .= " and TM_NAMAMITRA" . $arg_sum_tm_namamitra ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //----- 
   function Calc_resumo_groupbymitradatel($destino_resumo)
   {
      global $nm_lang, $prepare, $perijinan, $inst_testcomm, $closing, $pencapaian;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tp_releasename' => "TP_RELEASENAME",'tm_namamitra' => "TM_NAMAMITRA");
      $cmps_quebra_atual = array("tp_releasename","tm_namamitra");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_groupbymitradatel($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_groupbymitradatel($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(TP_RAB), sum(TP_NILAI), sum(TP_ODP), avg(REALISASI), max(REALISASI), min(REALISASI)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2), sum(SC_metric3), avg(SC_metric4), max(SC_metric4), min(SC_metric4)#@#cmps_quebras#@# from (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tm_namamitra']['reg'] = "S";
      $format_dimensions['tm_namamitra']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'groupbymitradatel', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_groupbymitradatel($where_ok);
   ...
   //----- 
   function Calc_resumo_gbmitraonly($destino_resumo)
   {
      global $nm_lang, $prepare, $perijinan, $inst_testcomm, $closing, $pencapaian;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tj_namajenis' => "TJ_NAMAJENIS");
      $cmps_quebra_atual = array("tj_namajenis");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_gbmitraonly($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_gbmitraonly($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      $format_dimensions = array();
      $format_dimensions['tj_namajenis']['reg'] = "S";
      $format_dimensions['tj_namajenis']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'gbmitraonly', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "tj_namajenis") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_gbmitraonly($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   //----- 
   function Calc_resumo_gbmitra($destino_resumo)
   {
      global $nm_lang, $prepare, $perijinan, $inst_testcomm, $closing, $pencapaian;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tm_namamitra' => "TM_NAMAMITRA");
      $cmps_quebra_atual = array("tm_namamitra");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_gbmitra($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['resume_realisasi_project_inwitel']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_gbmitra($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'gbmitra', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_gbmitra($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
