...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['project_edit_update_main_deleteproject']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="project_edit_update_main_deleteproject"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_deletedetail = false;
function sc_btn_deletedetail()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_deletedetail(); }, 500);
      return;
    }
    if (Crtl_btn_deletedetail) {return;}
    Crtl_btn_deletedetail = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?deletedetail?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_deleteproject = false;
function sc_btn_deleteproject()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_deleteproject(); }, 500);
      return;
    }
    if (Crtl_btn_deleteproject) {return;}
    Crtl_btn_deleteproject = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?deleteproject?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['project_edit_update_main_deleteproject']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_project_edit_update_main_deleteproject_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}
...
{ 
   ...
   document.F4.action               = "project_edit_update_main_deleteproject_fim.php";
   document.F4.submit(); 
} 
