...
$old_error_handler = set_error_handler("datel_scriptcase_error_handler");

function datel_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['datel']['contr_erro']) && $_SESSION['scriptcase']['datel']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_datel->contr_datel->Apl_com_erro = true;
            if ($inicial_datel->contr_datel->NM_ajax_flag)
            {
                $inicial_datel->contr_datel->NM_ajax_info['errList']['geral_datel'][] = $msg;
                return;
            }
            datel_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function datel_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['datel'])  && '' != $_SESSION['scriptcase']['error_icon']['datel'])  ? $_SESSION['scriptcase']['error_icon']['datel']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['datel']) && '' != $_SESSION['scriptcase']['error_close']['datel']) ? $_SESSION['scriptcase']['error_close']['datel'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['datel']) && '' != $_SESSION['scriptcase']['error_span_title']['datel']) ? $_SESSION['scriptcase']['error_span_title']['datel'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['datel']) && '' != $_SESSION['scriptcase']['error_icon_title']['datel']) ? $_SESSION['scriptcase']['error_icon_title']['datel'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['datel']['error_buffer'] .= ob_get_flush();
}
