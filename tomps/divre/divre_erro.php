...
$old_error_handler = set_error_handler("divre_scriptcase_error_handler");

function divre_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['divre']['contr_erro']) && $_SESSION['scriptcase']['divre']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_divre->contr_divre->Apl_com_erro = true;
            if ($inicial_divre->contr_divre->NM_ajax_flag)
            {
                $inicial_divre->contr_divre->NM_ajax_info['errList']['geral_divre'][] = $msg;
                return;
            }
            divre_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function divre_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['divre'])  && '' != $_SESSION['scriptcase']['error_icon']['divre'])  ? $_SESSION['scriptcase']['error_icon']['divre']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['divre']) && '' != $_SESSION['scriptcase']['error_close']['divre']) ? $_SESSION['scriptcase']['error_close']['divre'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['divre']) && '' != $_SESSION['scriptcase']['error_span_title']['divre']) ? $_SESSION['scriptcase']['error_span_title']['divre'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['divre']) && '' != $_SESSION['scriptcase']['error_icon_title']['divre']) ? $_SESSION['scriptcase']['error_icon_title']['divre'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['divre']['error_buffer'] .= ob_get_flush();
}
