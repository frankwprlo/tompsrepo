...
class dashboard_recentlyclose_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['dashboard_recentlyclose']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['dashboard_recentlyclose']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_groupbykatwaktu($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(TP_ID), sum(TP_ODP) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_groupbyjenisproject($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['contr_total_geral'] == "OK") 
      { 
   ...
   //-----  keterangan
   function quebra_keterangan_groupbykatwaktu($keterangan, $arg_sum_keterangan) 
   {
      global $tot_keterangan;
      $tot_keterangan = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(TP_ID), sum(TP_ODP) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where KETERANGAN" . $arg_sum_keterangan ; 
      } 
      else 
      { 
         $nm_comando .= " and KETERANGAN" . $arg_sum_keterangan ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_keterangan[0] = sc_strip_script($keterangan) ; 
      $tot_keterangan[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_keterangan[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_keterangan[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  tp_datel
   function quebra_tp_datel_groupbykatwaktu($keterangan, $tp_datel, $arg_sum_keterangan, $arg_sum_tp_datel) 
   {
      global $tot_tp_datel;
      $tot_tp_datel = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq'])) 
      { 
         $nm_comando .= " where KETERANGAN" . $arg_sum_keterangan . " and TP_DATEL" . $arg_sum_tp_datel ; 
      } 
      else 
      { 
         $nm_comando .= " and KETERANGAN" . $arg_sum_keterangan . " and TP_DATEL" . $arg_sum_tp_datel ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tp_datel[0] = sc_strip_script($tp_datel) ; 
      $tot_tp_datel[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tp_datel[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_tp_datel[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  md_kodedatel
   function quebra_md_kodedatel_groupbyjenisproject($md_kodedatel, $arg_sum_md_kodedatel) 
   {
      global $tot_md_kodedatel;
      $tot_md_kodedatel = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(TP_ID), sum(TP_ODP) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq'])) 
      { 
         $nm_comando .= " where MD_KODEDATEL" . $arg_sum_md_kodedatel ; 
      } 
      else 
      { 
         $nm_comando .= " and MD_KODEDATEL" . $arg_sum_md_kodedatel ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_md_kodedatel[0] = sc_strip_script($md_kodedatel) ; 
      $tot_md_kodedatel[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_md_kodedatel[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_md_kodedatel[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  tj_namajenis
   function quebra_tj_namajenis_groupbyjenisproject($md_kodedatel, $tj_namajenis, $arg_sum_md_kodedatel, $arg_sum_tj_namajenis) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq'])) 
      { 
         $nm_comando .= " where MD_KODEDATEL" . $arg_sum_md_kodedatel . " and TJ_NAMAJENIS" . $arg_sum_tj_namajenis ; 
      } 
      else 
      { 
         $nm_comando .= " and MD_KODEDATEL" . $arg_sum_md_kodedatel . " and TJ_NAMAJENIS" . $arg_sum_tj_namajenis ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //----- 
   function Calc_resumo_groupbykatwaktu($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('keterangan' => "KETERANGAN",'tp_datel' => "TP_DATEL");
      $cmps_quebra_atual = array("keterangan","tp_datel");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_groupbykatwaktu($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_groupbykatwaktu($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('tp_witel' => 'N','tp_divre' => 'N','tp_id' => 'N','tp_odp' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), count(TP_ID), sum(TP_ODP)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), count(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select TP_ID as SC_metric1,TP_ODP as SC_metric2, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['keterangan']['reg'] = "S";
      $format_dimensions['keterangan']['msk'] = "";
      $format_dimensions['tp_datel']['reg'] = "S";
      $format_dimensions['tp_datel']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'groupbykatwaktu', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "keterangan") {
              }
              if ($Cada_dim == "tp_datel") {
                  $this->Lookup->lookup_groupbykatwaktu_tp_datel($$Cada_dim,  $tp_datel);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_groupbykatwaktu($where_ok);
   ...
   //----- 
   function Calc_resumo_groupbyjenisproject($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('md_kodedatel' => "MD_KODEDATEL",'tj_namajenis' => "TJ_NAMAJENIS");
      $cmps_quebra_atual = array("md_kodedatel","tj_namajenis");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_groupbyjenisproject($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_recentlyclose']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_groupbyjenisproject($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      $format_dimensions = array();
      $format_dimensions['md_kodedatel']['reg'] = "S";
      $format_dimensions['md_kodedatel']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'groupbyjenisproject', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "md_kodedatel") {
              }
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_groupbyjenisproject($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
