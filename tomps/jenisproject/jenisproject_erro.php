...
$old_error_handler = set_error_handler("jenisproject_scriptcase_error_handler");

function jenisproject_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['jenisproject']['contr_erro']) && $_SESSION['scriptcase']['jenisproject']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_jenisproject->contr_jenisproject->Apl_com_erro = true;
            if ($inicial_jenisproject->contr_jenisproject->NM_ajax_flag)
            {
                $inicial_jenisproject->contr_jenisproject->NM_ajax_info['errList']['geral_jenisproject'][] = $msg;
                return;
            }
            jenisproject_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function jenisproject_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['jenisproject'])  && '' != $_SESSION['scriptcase']['error_icon']['jenisproject'])  ? $_SESSION['scriptcase']['error_icon']['jenisproject']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['jenisproject']) && '' != $_SESSION['scriptcase']['error_close']['jenisproject']) ? $_SESSION['scriptcase']['error_close']['jenisproject'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['jenisproject']) && '' != $_SESSION['scriptcase']['error_span_title']['jenisproject']) ? $_SESSION['scriptcase']['error_span_title']['jenisproject'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['jenisproject']) && '' != $_SESSION['scriptcase']['error_icon_title']['jenisproject']) ? $_SESSION['scriptcase']['error_icon_title']['jenisproject'] : '';
?>
