  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_jenisproject" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  // ---------- Validate tj_idjenis_
  function do_ajax_jenisproject_validate_tj_idjenis_(iNumLinha)
  {
    var nomeCampo_tj_idjenis_ = "tj_idjenis_" + iNumLinha;
    var var_tj_idjenis_ = scAjaxGetFieldHidden(nomeCampo_tj_idjenis_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_jenisproject_validate_tj_idjenis_(var_tj_idjenis_, iNumLinha, var_script_case_init, do_ajax_jenisproject_validate_tj_idjenis__cb);
  } // do_ajax_jenisproject_validate_tj_idjenis_

  function do_ajax_jenisproject_validate_tj_idjenis__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "tj_idjenis_" + iLineNumber;
    }
    else
    {
      sFieldValid = "tj_idjenis_";
    }
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "tj_idjenis_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "tj_idjenis_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_jenisproject_validate_tj_idjenis__cb

  // ---------- Validate tj_namajenis_
  function do_ajax_jenisproject_validate_tj_namajenis_(iNumLinha)
  {
    var nomeCampo_tj_namajenis_ = "tj_namajenis_" + iNumLinha;
    var var_tj_namajenis_ = scAjaxGetFieldText(nomeCampo_tj_namajenis_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_jenisproject_validate_tj_namajenis_(var_tj_namajenis_, iNumLinha, var_script_case_init, do_ajax_jenisproject_validate_tj_namajenis__cb);
  } // do_ajax_jenisproject_validate_tj_namajenis_

  function do_ajax_jenisproject_validate_tj_namajenis__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "tj_namajenis_" + iLineNumber;
    }
    else
    {
      sFieldValid = "tj_namajenis_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "tj_namajenis_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "tj_namajenis_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_jenisproject_validate_tj_namajenis__cb

  // ---------- Validate tj_deskripsi_
  function do_ajax_jenisproject_validate_tj_deskripsi_(iNumLinha)
  {
    var nomeCampo_tj_deskripsi_ = "tj_deskripsi_" + iNumLinha;
    var var_tj_deskripsi_ = scAjaxGetFieldText(nomeCampo_tj_deskripsi_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_jenisproject_validate_tj_deskripsi_(var_tj_deskripsi_, iNumLinha, var_script_case_init, do_ajax_jenisproject_validate_tj_deskripsi__cb);
  } // do_ajax_jenisproject_validate_tj_deskripsi_

  function do_ajax_jenisproject_validate_tj_deskripsi__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "tj_deskripsi_" + iLineNumber;
    }
    else
    {
      sFieldValid = "tj_deskripsi_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "tj_deskripsi_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "tj_deskripsi_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_jenisproject_validate_tj_deskripsi__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_jenisproject_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_jenisproject_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_jenisproject_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_jenisproject_add_new_line_cb);
  } // do_ajax_jenisproject_add_new_line

  function do_ajax_jenisproject_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
  ...
  } // do_ajax_jenisproject_add_new_line_cb

  // ---------- backup_line
  function do_ajax_jenisproject_backup_line(iNumLinha)
  {
    var var_tj_idjenis_ = scAjaxGetFieldHidden("tj_idjenis_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_jenisproject_backup_line(var_tj_idjenis_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_jenisproject_backup_line_cb);
  } // do_ajax_jenisproject_backup_line

  function do_ajax_jenisproject_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    if (!scAjaxHasError())
    {
  ...
  } // do_ajax_jenisproject_backup_line_cb

  function do_ajax_jenisproject_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_jenisproject_cancel_insert

  function do_ajax_jenisproject_cancel_update(iSeqVert)
  {
    do_ajax_jenisproject_backup_line(iSeqVert);
...
?>
  } // do_ajax_jenisproject_cancel_update

  function do_ajax_jenisproject_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_jenisproject_restore_buttons

  // ---------- table_refresh
  function do_ajax_jenisproject_table_refresh()
  {
    var var_tj_idjenis_ = document.F2.tj_idjenis_.value;
    ...
    scAjaxProcOn();
    x_ajax_jenisproject_table_refresh(var_tj_idjenis_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_jenisproject_table_refresh_cb);
  } //  do_ajax_jenisproject_table_refresh

  function do_ajax_jenisproject_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    document.F2.tj_idjenis_.value = scAjaxGetKeyValue("tj_idjenis_");
    ...
    sc_insert_open = false;
  } // do_ajax_jenisproject_table_refresh_cb

  // ---------- Form
  var sc_num_ult_line = "";
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_jenisproject_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_jenisproject_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    scAjaxHideMessage();
    var var_tj_idjenis_ = scAjaxGetFieldHidden("tj_idjenis_" + iNumLinha);
    var var_tj_namajenis_ = scAjaxGetFieldText("tj_namajenis_" + iNumLinha);
    var var_tj_deskripsi_ = scAjaxGetFieldText("tj_deskripsi_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_jenisproject_submit_form(var_tj_idjenis_, var_tj_namajenis_, var_tj_deskripsi_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_jenisproject_submit_form_cb);
  } // do_ajax_jenisproject_submit_form

  function do_ajax_jenisproject_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxShowMessage();
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("tj_idjenis_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("tj_namajenis_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("tj_deskripsi_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetFields();
...
?>
  ...
  } // do_ajax_jenisproject_submit_form_cb

  var scStatusDetail = {};

  function do_ajax_jenisproject_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_jenisproject_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("tj_idjenis_" + iNavForm);
      scAjaxHideErrorDisplay("tj_namajenis_" + iNavForm);
      scAjaxHideErrorDisplay("tj_deskripsi_" + iNavForm);
    }
    var var_tj_idjenis_ = document.F2.tj_idjenis_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_jenisproject_navigate_form(var_tj_idjenis_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_jenisproject_navigate_form_cb);
  } // do_ajax_jenisproject_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['jenisproject']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_jenisproject_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    if (oResp['empty_filter'] && oResp['empty_filter'] == "ok")
    ...
    sc_mupload_ok = true;
    scAjaxSetFields();
    document.F2.tj_idjenis_.value = scAjaxGetKeyValue("tj_idjenis_" + var_last_index);
...
?>
  ...
  } // do_ajax_jenisproject_navigate_form_cb
  function sc_hide_jenisproject_form()
  {
    for (var block_id in ajax_block_id) {
      $("#div_" + ajax_block_id[block_id]).hide();
    }
  } // sc_hide_jenisproject_form
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "tj_idjenis_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "tj_namajenis_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "tj_deskripsi_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["tj_idjenis_" + iNumLinha] = {"label": "ID<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["tj_namajenis_" + iNumLinha] = {"label": "Nama<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["tj_deskripsi_" + iNumLinha] = {"label": "Deskripsi<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["tj_idjenis_"][iNumLinha] = "tj_idjenis_" + iNumLinha;
    ajax_field_mult["tj_namajenis_"][iNumLinha] = "tj_namajenis_" + iNumLinha;
    ajax_field_mult["tj_deskripsi_"][iNumLinha] = "tj_deskripsi_" + iNumLinha;
    ajax_field_id["tj_idjenis_" + iNumLinha] = new Array("hidden_field_label_tj_idjenis_", "hidden_field_data_tj_idjenis_" + iNumLinha);
    ajax_field_id["tj_namajenis_" + iNumLinha] = new Array("hidden_field_label_tj_namajenis_", "hidden_field_data_tj_namajenis_" + iNumLinha);
    ajax_field_id["tj_deskripsi_" + iNumLinha] = new Array("hidden_field_label_tj_deskripsi_", "hidden_field_data_tj_deskripsi_" + iNumLinha);
    ajax_error_count["tj_idjenis_" + iNumLinha] = "off";
    ajax_error_count["tj_namajenis_" + iNumLinha] = "off";
    ajax_error_count["tj_deskripsi_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["tj_idjenis_" + iNumLinha] = "on";
    ajax_read_only["tj_namajenis_" + iNumLinha] = "off";
    ajax_read_only["tj_deskripsi_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["tj_idjenis_" + iNumLinha] = "on";
    ajax_read_only["tj_namajenis_" + iNumLinha] = "on";
    ajax_read_only["tj_deskripsi_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["tj_idjenis_" + iNumLinha] = null;
    ajax_error_list["tj_namajenis_" + iNumLinha] = null;
    ajax_error_list["tj_deskripsi_" + iNumLinha] = null;
  }
  ...
  var ajax_field_mult = {
    "tj_idjenis_": new Array(),
    "tj_namajenis_": new Array(),
    "tj_deskripsi_": new Array()
  };
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["tj_idjenis_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["tj_idjenis_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["tj_idjenis_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["tj_idjenis_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["tj_idjenis_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["tj_namajenis_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["tj_namajenis_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["tj_namajenis_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["tj_namajenis_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["tj_namajenis_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["tj_deskripsi_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["tj_deskripsi_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["tj_deskripsi_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["tj_deskripsi_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["tj_deskripsi_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['tj_idjenis_'])) ? $this->nmgp_cmp_readonly['tj_idjenis_'] : 'on';
?>
    scAjaxFieldRead("tj_idjenis_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['tj_namajenis_'])) ? $this->nmgp_cmp_readonly['tj_namajenis_'] : 'off';
?>
    scAjaxFieldRead("tj_namajenis_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['tj_deskripsi_'])) ? $this->nmgp_cmp_readonly['tj_deskripsi_'] : 'off';
?>
    scAjaxFieldRead("tj_deskripsi_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("tj_idjenis_" + iSeq, "on");
    scAjaxFieldRead("tj_namajenis_" + iSeq, "on");
    scAjaxFieldRead("tj_deskripsi_" + iSeq, "on");
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["tj_idjenis_" + iRow] || "on" == ajax_error_count["tj_namajenis_" + iRow] || "on" == ajax_error_count["tj_deskripsi_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_tj_idjenis_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_tj_namajenis_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_tj_deskripsi_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["tj_idjenis_" + iRow] && "off" == ajax_error_count["tj_namajenis_" + iRow] && "off" == ajax_error_count["tj_deskripsi_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["tj_idjenis_" + iRow] = "off";
        ajax_error_count["tj_namajenis_" + iRow] = "off";
        ajax_error_count["tj_deskripsi_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_tj_idjenis_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_tj_namajenis_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_tj_deskripsi_" + iRow).removeClass("scFormErrorLine");
    }
  }
