<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="tp_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['project_edit_update_main_applink']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="project_edit_update_main_applink"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_generate_odp_var_projectid = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_id']); ?>";
var var_btn_generate_odp_var_lop = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_idlop']); ?>";
var var_btn_generate_odp_var_namaproject = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_projectname']); ?>";
var var_btn_generate_odp_var_lokasi = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_lokasi']); ?>";
var var_btn_generate_odp_namawitel = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_witel']); ?>";
var var_btn_generate_odp_namasto = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_sto']); ?>";
function sc_btn_generate_odp()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_generate_odp(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_form_master_odp_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_projectid*scin' + var_btn_generate_odp_var_projectid + '*scoutvar_lop*scin' + var_btn_generate_odp_var_lop + '*scoutvar_namaproject*scin' + var_btn_generate_odp_var_namaproject + '*scoutvar_lokasi*scin' + var_btn_generate_odp_var_lokasi + '*scoutnamawitel*scin' + var_btn_generate_odp_namawitel + '*scoutnamasto*scin' + var_btn_generate_odp_namasto + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'form_master_odp');
}
var var_btn_generate_odp_new_var_projectid = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_id']); ?>";
function sc_btn_generate_odp_new()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_generate_odp_new(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_project_edit_update_main_odpedit_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_projectid*scin' + var_btn_generate_odp_new_var_projectid + '*scout"; ?>', '', '_self', '0', '0', 'project_edit_update_main_odpedit');
}
var var_btn_progresstask_var_idproject = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_id']); ?>";
var var_btn_progresstask_var_var_tglmulaiproject = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_planstart']); ?>";
var var_btn_progresstask_var_tglmulaibaru = "<?php echo $this->form_encode_input($this->nmgp_dados_form['tp_planstart']); ?>";
function sc_btn_progresstask()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_progresstask(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_project_edit_update_main_jadwal_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_idproject*scin' + var_btn_progresstask_var_idproject + '*scoutvar_var_tglmulaiproject*scin' + var_btn_progresstask_var_var_tglmulaiproject + '*scoutvar_tglmulaibaru*scin' + var_btn_progresstask_var_tglmulaibaru + '*scout"; ?>', 'inicio', '_self', '0', '0', 'project_edit_update_main_jadwal');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['project_edit_update_main_applink']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_project_edit_update_main_applink_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}
