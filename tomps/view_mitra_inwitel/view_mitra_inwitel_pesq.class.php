...
class view_mitra_inwitel_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . view_mitra_inwitel_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . view_mitra_inwitel_pack_protect_string($Tipo_filter[0]) . "\">.." . view_mitra_inwitel_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == "ajax_refresh_field")
      {
          ...
          if (in_array("tm_witel", $NMcmp_refr))
          {
              $list = array();
              $nmgp_def_dados = $this->lookup_ajax_tm_witel();
              ...
              $this->Arr_result['set_option'][] = array('field' => 'SC_tm_witel', 'value' => $list);
          }
      }
      if ($this->NM_ajax_opcao == 'autocomp_tm_namamitra')
      {
          $tm_namamitra = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_tm_namamitra($tm_namamitra);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_tm_witel()
   {
            $tm_witel_look = substr($this->Db->qstr($tm_witel), 1, -1); 
      $nmgp_def_dados = array(); 
      $nmgp_def_dados[] = array("" => ""); 
      $nm_comando = "SELECT MW_IDWITEL, MW_NAMAWITEL  FROM TBL_MAS_WITEL WHERE MW_IDDIVRE=" . $_SESSION['TM_DIVRE'] . " ORDER BY MW_NAMAWITEL"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_witel'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_witel'][] = trim($rs->fields[0]);
   ...
   function lookup_ajax_tm_namamitra($tm_namamitra)
   {
      $tm_namamitra = substr($this->Db->qstr($tm_namamitra), 1, -1);
            $tm_namamitra_look = substr($this->Db->qstr($tm_namamitra), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct TM_NAMAMITRA from " . $this->Ini->nm_tabela . " where  TM_NAMAMITRA like '%" . $tm_namamitra . "%' order by TM_NAMAMITRA"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      $Nm_datas[] = "TM_ENTRYDATE";$Nm_datas[] = "TM_UPDATEDATE";
      if (in_array($campo_join, $Nm_datas))
      {
          ...
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['SC_sep_date']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['SC_sep_date']))
          {
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - ""</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>view_mitra_inwitel/view_mitra_inwitel_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="view_mitra_inwitel_ajax_search.js"></script>
 <script type="text/javascript" src="view_mitra_inwitel_ajax.js"></script>
 <script type="text/javascript">
...
function NM_hide_menu()
{
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_tm_divre_cond#NMF#' + search_get_sel_txt('SC_tm_divre_cond') + '@NMF@';
  str_out += 'SC_tm_divre#NMF#' + search_get_select('SC_tm_divre') + '@NMF@';
  str_out += 'SC_tm_witel_cond#NMF#' + search_get_sel_txt('SC_tm_witel_cond') + '@NMF@';
  str_out += 'SC_tm_witel#NMF#' + search_get_select('SC_tm_witel') + '@NMF@';
  str_out += 'SC_tm_namamitra_cond#NMF#' + search_get_sel_txt('SC_tm_namamitra_cond') + '@NMF@';
  str_out += 'SC_tm_namamitra#NMF#' + search_get_text('SC_tm_namamitra') + '@NMF@';
  str_out += 'id_ac_tm_namamitra#NMF#' + search_get_text('id_ac_tm_namamitra') + '@NMF@';
 ...
 function nm_refresh_tm_divre(Proc_on)
 {
   var parms  = "";
   var fields = "tm_witel";
...
<script type="text/javascript">
 $(function() {
   $("#id_ac_tm_namamitra").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_tm_namamitra",
          max_itens: "10",
          cod_desc: "N",
     ...
     select: function (event, ui) {
       $("#SC_tm_namamitra").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_tm_namamitra").val( $(this).val() );
       }
     }
   });
 });
...
<?php
   ...
   {
      global 
             $tm_divre_cond, $tm_divre,
             $tm_witel_cond, $tm_witel,
             $tm_namamitra_cond, $tm_namamitra, $tm_namamitra_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $tm_divre = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_divre']; 
          $tm_divre_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_divre_cond']; 
          $tm_witel = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_witel']; 
          $tm_witel_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_witel_cond']; 
          $tm_namamitra = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_namamitra']; 
          $tm_namamitra_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_namamitra_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['NM_operador']; 
      } 
      if (!isset($tm_divre_cond) || empty($tm_divre_cond))
      {
         $tm_divre_cond = "eq";
      }
      if (!isset($tm_witel_cond) || empty($tm_witel_cond))
      {
         $tm_witel_cond = "eq";
      }
      ...
      $str_hide_tm_divre = (in_array($tm_divre_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_tm_witel = (in_array($tm_witel_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_tm_namamitra = (in_array($tm_namamitra_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($tm_divre) || $tm_divre == "")
      {
          $tm_divre = "";
      }
      if (isset($tm_divre) && !empty($tm_divre))
      {
         $tmp_pos = strpos($tm_divre, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $tm_divre = substr($tm_divre, 0, $tmp_pos);
         }
      }
      if (!isset($tm_witel) || $tm_witel == "")
      {
          $tm_witel = "";
      }
      if (isset($tm_witel) && !empty($tm_witel))
      {
         $tmp_pos = strpos($tm_witel, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $tm_witel = substr($tm_witel, 0, $tmp_pos);
         }
      }
      if (!isset($tm_namamitra) || $tm_namamitra == "")
      {
          $tm_namamitra = "";
      }
      if (isset($tm_namamitra) && !empty($tm_namamitra))
      {
         $tmp_pos = strpos($tm_namamitra, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $tm_namamitra = substr($tm_namamitra, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD class="scFilterLabelOdd"><?php echo (isset($this->New_label['tm_divre'])) ? $this->New_label['tm_divre'] : "TM DIVRE"; ?></TD>
      
      <INPUT type="hidden" id="SC_tm_divre_cond" name="tm_divre_cond" value="eq">
 
     <TD colspan=2 class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_tm_divre" <?php echo $str_hide_tm_divre?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['tm_divre'])) ? $this->New_label['tm_divre'] : "TM DIVRE";
 $nmgp_tab_label .= "tm_divre?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>

<?php
      $tm_divre_look = substr($this->Db->qstr($tm_divre), 1, -1); 
      $nmgp_def_dados = "" ; 
      ...
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_divre'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_divre'][] = trim($rs->fields[0]);
...
?>
   <span id="idAjaxSelect_tm_divre">
      <SELECT class="scFilterObjectOdd" id="SC_tm_divre" name="tm_divre" onChange="nm_refresh_tm_divre();" size="1">
       <OPTION value=""></OPTION>
<?php
      ...
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $tm_divre)
            {
                    $tm_divre_sel = ($nm_opc_cod === $tm_divre) ? "selected" : "";
            }
            else
            {
               $tm_divre_sel = ("S" == $nm_opc_sel) ? "selected" : "";
            }
            $nm_sc_valor = $nm_opc_val;
            $nm_opc_val = $nm_sc_valor;
?>
       <OPTION value="<?php echo NM_encode_input($nm_opc_cod . $delimitador . $nm_opc_val); ?>" <?php echo $tm_divre_sel; ?>><?php echo $nm_opc_val; ?></OPTION>
<?php
         }
      }
?>
...
?>
      ...
      <TD class="scFilterLabelEven"><?php echo (isset($this->New_label['tm_witel'])) ? $this->New_label['tm_witel'] : "TM WITEL"; ?></TD>
      
      <INPUT type="hidden" id="SC_tm_witel_cond" name="tm_witel_cond" value="eq">
 
     <TD colspan=2 class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_tm_witel" <?php echo $str_hide_tm_witel?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['tm_witel'])) ? $this->New_label['tm_witel'] : "TM WITEL";
 $nmgp_tab_label .= "tm_witel?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
...
?>
   <span id="idAjaxSelect_tm_witel">
      <SELECT class="scFilterObjectEven" id="SC_tm_witel" name="tm_witel"  size="1">
       <OPTION value=""></OPTION>
<?php
      $nm_opcoesx = str_replace("?#?@?#?", "?#?@ ?#?", $nmgp_def_dados);
      $nm_opcoes  = explode("?@?", $nm_opcoesx);
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $tm_witel)
            {
                    $tm_witel_sel = ($nm_opc_cod === $tm_witel) ? "selected" : "";
            }
            else
            {
               $tm_witel_sel = ("S" == $nm_opc_sel) ? "selected" : "";
            }
            $nm_sc_valor = $nm_opc_val;
            $nm_opc_val = $nm_sc_valor;
?>
       <OPTION value="<?php echo NM_encode_input($nm_opc_cod . $delimitador . $nm_opc_val); ?>" <?php echo $tm_witel_sel; ?>><?php echo $nm_opc_val; ?></OPTION>
<?php
         }
      }
?>
...
?>
      ...
      <TD class="scFilterLabelOdd"><?php echo (isset($this->New_label['tm_namamitra'])) ? $this->New_label['tm_namamitra'] : "TM NAMAMITRA"; ?></TD>
      
      <INPUT type="hidden" id="SC_tm_namamitra_cond" name="tm_namamitra_cond" value="eq">
 
     <TD colspan=2 class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_tm_namamitra" <?php echo $str_hide_tm_namamitra?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['tm_namamitra'])) ? $this->New_label['tm_namamitra'] : "TM NAMAMITRA";
 $nmgp_tab_label .= "tm_namamitra?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      if ($tm_namamitra != "")
      ...
      $tm_namamitra_look = substr($this->Db->qstr($tm_namamitra), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct TM_NAMAMITRA from " . $this->Ini->nm_tabela . " where TM_NAMAMITRA = '$tm_namamitra_look' order by TM_NAMAMITRA"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
      ...
      if (isset($nmgp_def_dados[0][$tm_namamitra]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$tm_namamitra];
      }
      else
      {
          $sAutocompValue = $tm_namamitra;
      }
?>
<INPUT  type="text" id="SC_tm_namamitra" name="tm_namamitra" value="<?php echo NM_encode_input($tm_namamitra) ?>" size=50 alt="{datatype: 'text', maxLength: 60, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_tm_namamitra" name="tm_namamitra_autocomp" size="50" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 50, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (is_file("view_mitra_inwitel_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['view_mitra_inwitel']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['view_mitra_inwitel']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   nm_campos_between(document.getElementById('id_vis_tm_divre'), document.F1.tm_divre_cond, 'tm_divre');
   document.F1.tm_divre.value = "";
   nm_campos_between(document.getElementById('id_vis_tm_witel'), document.F1.tm_witel_cond, 'tm_witel');
   document.F1.tm_witel.value = "";
   nm_campos_between(document.getElementById('id_vis_tm_namamitra'), document.F1.tm_namamitra_cond, 'tm_namamitra');
   document.F1.tm_namamitra.value = "";
   document.F1.tm_namamitra_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "projectamo/view_mitra_inwitel";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro()
   {
      ...
      $tp_fields     = array();
      $tb_fields_esp = array();
      $tp_fields['SC_tm_divre_cond'] = 'cond';
      $tp_fields['SC_tm_divre'] = 'select';
      $tp_fields['SC_tm_witel_cond'] = 'cond';
      $tp_fields['SC_tm_witel'] = 'select';
      $tp_fields['SC_tm_namamitra_cond'] = 'cond';
      $tp_fields['SC_tm_namamitra'] = 'text_aut';
      $tp_fields['id_ac_tm_namamitra'] = 'text_aut';
      if (is_file($NM_patch))
      {
          $SC_V8    = false;
   ...
   {
      global $tm_divre_cond, $tm_divre,
             $tm_witel_cond, $tm_witel,
             $tm_namamitra_cond, $tm_namamitra, $tm_namamitra_autocomp, $nmgp_tab_label;

      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($tm_namamitra_autocomp) && empty($tm_namamitra))
      {
          $tm_namamitra = $tm_namamitra_autocomp;
      }
      $tm_divre_cond_salva = $tm_divre_cond; 
      if (!isset($tm_divre_input_2) || $tm_divre_input_2 == "")
      {
          $tm_divre_input_2 = $tm_divre;
      }
      $tm_witel_cond_salva = $tm_witel_cond; 
      if (!isset($tm_witel_input_2) || $tm_witel_input_2 == "")
      {
          $tm_witel_input_2 = $tm_witel;
      }
      $tm_namamitra_cond_salva = $tm_namamitra_cond; 
      if (!isset($tm_namamitra_input_2) || $tm_namamitra_input_2 == "")
      {
          $tm_namamitra_input_2 = $tm_namamitra;
      }
      $tmp_pos = strpos($tm_divre, "##@@");
      if ($tmp_pos === false) {
          $L_lookup = $tm_divre;
          ...
          $L_lookup = substr($tm_divre, 0, $tmp_pos);
      }
      if (!empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_divre'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "TM DIVRE : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      $tmp_pos = strpos($tm_witel, "##@@");
      if ($tmp_pos === false) {
          $L_lookup = $tm_witel;
          ...
          $L_lookup = substr($tm_witel, 0, $tmp_pos);
      }
      if (!empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['psq_check_ret']['tm_witel'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "TM WITEL : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_divre'] = $tm_divre; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_divre_cond'] = $tm_divre_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_witel'] = $tm_witel; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_witel_cond'] = $tm_witel_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_namamitra'] = $tm_namamitra; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['tm_namamitra_cond'] = $tm_namamitra_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if (!empty($this->Campos_Mens_erro)) 
      {
          return;
      }
      ...
      $this->cmp_formatado['tm_divre'] = $Conteudo;
      $Conteudo = $tm_witel;
      if (strpos($Conteudo, "##@@") !== false)
      {
          $Conteudo = substr($Conteudo, strpos($Conteudo, "##@@") + 4);
      }
      $this->cmp_formatado['tm_witel'] = $Conteudo;
      $tm_namamitra_look = substr($this->Db->qstr($tm_namamitra), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct TM_NAMAMITRA from " . $this->Ini->nm_tabela . " where TM_NAMAMITRA = '$tm_namamitra_look' order by TM_NAMAMITRA"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['tm_namamitra'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['tm_namamitra'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['tm_namamitra'] = $tm_namamitra;
      }
      ...
      if (isset($tm_divre))
      {
         $this->monta_condicao("TM_DIVRE", $tm_divre_cond, $tm_divre, "", "tm_divre");
      }
      ...
      if (isset($tm_witel))
      {
         $this->monta_condicao("TM_WITEL", $tm_witel_cond, $tm_witel, "", "tm_witel");
      }

      //----- $tm_namamitra
      $this->Date_part = false;
      if (isset($tm_namamitra) || $tm_namamitra_cond == "nu" || $tm_namamitra_cond == "nn" || $tm_namamitra_cond == "ep" || $tm_namamitra_cond == "ne")
      {
         $this->monta_condicao("TM_NAMAMITRA", $tm_namamitra_cond, $tm_namamitra, "", "tm_namamitra");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['cond_pesq'] .= $this->NM_operador;
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['view_mitra_inwitel']['where_pesq_ant'] = $this->comando . $this->comando_fim;
   ...
   function css_obj_select_ajax($Obj)
   {
      switch ($Obj)
      {
         case "tm_divre" : return ('class="scFilterObjectOdd"'); break;
         case "tm_witel" : return ('class="scFilterObjectEven"'); break;
         default       : return ("");
      }
   }
