...
$old_error_handler = set_error_handler("batch_scriptcase_error_handler");

function batch_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['batch']['contr_erro']) && $_SESSION['scriptcase']['batch']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_batch->contr_batch->Apl_com_erro = true;
            if ($inicial_batch->contr_batch->NM_ajax_flag)
            {
                $inicial_batch->contr_batch->NM_ajax_info['errList']['geral_batch'][] = $msg;
                return;
            }
            batch_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function batch_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['batch'])  && '' != $_SESSION['scriptcase']['error_icon']['batch'])  ? $_SESSION['scriptcase']['error_icon']['batch']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['batch']) && '' != $_SESSION['scriptcase']['error_close']['batch']) ? $_SESSION['scriptcase']['error_close']['batch'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['batch']) && '' != $_SESSION['scriptcase']['error_span_title']['batch']) ? $_SESSION['scriptcase']['error_span_title']['batch'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['batch']) && '' != $_SESSION['scriptcase']['error_icon_title']['batch']) ? $_SESSION['scriptcase']['error_icon_title']['batch'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['batch']['error_buffer'] .= ob_get_flush();
}
