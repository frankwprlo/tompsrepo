<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idtable" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idtable']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tahapanproject']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tahapanproject"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tahapanproject']['where_detal']))
  {
?>  
...
function lookup_templateproject(campo)
{
   dados  = document.F1.templateproject.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_tahapanproject')?>/form_tahapanproject.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_templateproject, "templateproject", dados, false);
}
function recebe_templateproject(string)
{
   document.getElementById('id_lookup_templateproject').innerHTML = string;
}
function lookup_projectphaseid(campo)
{
   dados  = document.F1.projectphaseid.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_tahapanproject')?>/form_tahapanproject.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_projectphaseid, "projectphaseid", dados, false);
}
function recebe_projectphaseid(string)
{
   document.getElementById('id_lookup_projectphaseid').innerHTML = string;
}
<?php 
