...
$old_error_handler = set_error_handler("witel_scriptcase_error_handler");

function witel_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['witel']['contr_erro']) && $_SESSION['scriptcase']['witel']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_witel->contr_witel->Apl_com_erro = true;
            if ($inicial_witel->contr_witel->NM_ajax_flag)
            {
                $inicial_witel->contr_witel->NM_ajax_info['errList']['geral_witel'][] = $msg;
                return;
            }
            witel_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function witel_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['witel'])  && '' != $_SESSION['scriptcase']['error_icon']['witel'])  ? $_SESSION['scriptcase']['error_icon']['witel']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['witel']) && '' != $_SESSION['scriptcase']['error_close']['witel']) ? $_SESSION['scriptcase']['error_close']['witel'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['witel']) && '' != $_SESSION['scriptcase']['error_span_title']['witel']) ? $_SESSION['scriptcase']['error_span_title']['witel'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['witel']) && '' != $_SESSION['scriptcase']['error_icon_title']['witel']) ? $_SESSION['scriptcase']['error_icon_title']['witel'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['witel']['error_buffer'] .= ob_get_flush();
}
