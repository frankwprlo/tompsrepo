<?php
   ...
   session_start();
    ...
    $Sel_Cmp = new monitoring_lop_inregional_sel_cmp(); 
    $Sel_Cmp->Sel_cmp_init();
   
class monitoring_lop_inregional_sel_cmp
{
function Sel_cmp_init()
{
   ...
   $tab_blk_campos[] = "tp_batch";
   $tab_ger_campos['tp_witel'] = "on";
   $tab_blk_campos[] = "tp_witel";
   $tab_ger_campos['tp_datel'] = "on";
   $tab_blk_campos[] = "tp_datel";
   $tab_ger_campos['tp_sto'] = "on";
   $tab_blk_campos[] = "tp_sto";
   ...
   $tab_ger_campos['remainsnew'] = "on";
   $tab_blk_campos[] = "remainsnew";
   $tab_ger_campos['tp_status'] = "on";
   $tab_blk_campos[] = "tp_status";
   $tab_ger_campos['kickoff_meeting'] = "on";
   $tab_ger_campos['survey'] = "on";
   $tab_ger_campos['rab_apd_kml'] = "on";
   $tab_ger_campos['njki_justifikasi_nodin'] = "on";
   $tab_ger_campos['perijinan'] = "on";
   $tab_ger_campos['delivery_material'] = "on";
   $tab_ger_campos['tcbr'] = "on";
   $tab_ger_campos['hdpe_tanam_tiang'] = "on";
   $tab_ger_campos['gelar_kabel'] = "on";
   $tab_ger_campos['pasang_terminal_odf'] = "on";
   $tab_ger_campos['pasang_terminal_odc'] = "on";
   $tab_ger_campos['pasang_terminal_odp'] = "on";
   $tab_ger_campos['terminasi'] = "on";
   $tab_ger_campos['test_comm'] = "on";
   $tab_ger_campos['uji_terima'] = "on";
   $tab_ger_campos['rekon_amandemen'] = "on";
   $tab_ger_campos['baut_bast'] = "on";
   $tab_ger_campos['gr_ir'] = "on";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel'] = array();
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['usr_cmp_sel'] = $arr_temp;
?>
...
?>
  function scSubmitSelCampos(sPos) {
   ...
   $.ajax({
    type: "POST",
    url: "monitoring_lop_inregional_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['labels'][$NM_cada_field];
           }
       }
   }
...
<?php
   if (isset($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               ...
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsEnabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['labels'][$NM_cada_field]; ?></li>
<?php
               }
               else
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsDisabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['monitoring_lop_inregional']['labels'][$NM_cada_field]; ?></li>
<?php
               }
           }
       }
