...
class dashboard_alarm_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['dashboard_alarm']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['dashboard_alarm']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(TP_ID) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['contr_total_geral'] = "OK";
   } 

   //-----  tp_datel
   function quebra_tp_datel_groupbywiteldanmitra($tp_datel, $arg_sum_tp_datel) 
   {
      global $tot_tp_datel;
      $tot_tp_datel = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      ...
      { 
         $nm_comando .= " where TP_DATEL" . $arg_sum_tp_datel ; 
      } 
      else 
      { 
         $nm_comando .= " and TP_DATEL" . $arg_sum_tp_datel ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tp_datel[0] = NM_encode_input(sc_strip_script($tp_datel)) ; 
   ...
   //-----  tp_mitra
   function quebra_tp_mitra_groupbywiteldanmitra($tp_datel, $tp_mitra, $arg_sum_tp_datel, $arg_sum_tp_mitra) 
   {
      global $tot_tp_mitra;
      $tot_tp_mitra = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['where_pesq'])) 
      { 
         $nm_comando .= " where TP_DATEL" . $arg_sum_tp_datel . " and TP_MITRA" . $arg_sum_tp_mitra ; 
      } 
      else 
      { 
         $nm_comando .= " and TP_DATEL" . $arg_sum_tp_datel . " and TP_MITRA" . $arg_sum_tp_mitra ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tp_mitra[0] = NM_encode_input(sc_strip_script($tp_mitra)) ; 
      $tot_tp_mitra[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tp_mitra[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 
   ...
   function Calc_resumo_groupbywiteldanmitra($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tp_datel' => "TP_DATEL",'tp_mitra' => "TP_MITRA");
      $cmps_quebra_atual = array("tp_datel","tp_mitra");
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_groupbywiteldanmitra($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('tp_divre' => 'N','tp_witel' => 'N','tp_datel' => 'N','tp_id' => 'N','tp_mitra' => 'N','tp_odp' => 'N','sisawaktu' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), count(TP_ID)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), count(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select TP_ID as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['dashboard_alarm']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tp_mitra']['reg'] = "S";
      $format_dimensions['tp_mitra']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "tp_datel") {
                  $this->Lookup->lookup_groupbywiteldanmitra_tp_datel($$Cada_dim,  $tp_datel);
              }
              if ($Cada_dim == "tp_mitra") {
                  $this->Lookup->lookup_groupbywiteldanmitra_tp_mitra($$Cada_dim,  $tp_mitra);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
